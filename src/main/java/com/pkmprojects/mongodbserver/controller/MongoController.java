...
public class MongoController {
    ...
    public MongoController(ProvisioningService provisioningService, ExplorationService explorationService,
    ...
    public String home(Model model) {
        model.addAttribute("databases", provisioningService.listDatabases(DatabaseEngineType.MONGO));
    ...
    public String provision(@Valid @ModelAttribute("form") CreateDatabaseForm form,
        ...
        CreateDatabaseForm withEngine = new CreateDatabaseForm(form.dbName(), DatabaseEngineType.MONGO, form.userName(), form.password());
        ...
        return "redirect:/mongo/databases/" + created.dbName();
    }

    @GetMapping("/databases/{dbName}")
    public String detail(@PathVariable String dbName, Model model) {
        model.addAttribute("database", provisioningService.getDatabase(DatabaseEngineType.MONGO, dbName));
    ...
    public String resetPassword(@PathVariable String dbName,
        ...
        DatabaseInfo updated = provisioningService.resetPassword(DatabaseEngineType.MONGO, dbName, form);
        ...
        return "redirect:/mongo/databases/" + dbName;
    }
    ...
    public String delete(@PathVariable String dbName, RedirectAttributes redirectAttributes) {
        provisioningService.delete(DatabaseEngineType.MONGO, dbName);
    ...
    public String users(@PathVariable String dbName, Model model) {
        model.addAttribute("database", provisioningService.getDatabase(DatabaseEngineType.MONGO, dbName));
        model.addAttribute("users", provisioningService.listUsers(DatabaseEngineType.MONGO, dbName));
        model.addAttribute("engine", DatabaseEngineType.MONGO);
        return "users";
    }
    ...
    public String revokeUser(@PathVariable String dbName, @PathVariable String userName, RedirectAttributes redirectAttributes) {
        provisioningService.revokeUser(DatabaseEngineType.MONGO, dbName, userName);
        redirectAttributes.addFlashAttribute("flashSuccess", "User '" + userName + "' revoked from database '" + dbName + "'");
        return "redirect:/mongo/databases/" + dbName + "/users";
    }
}
