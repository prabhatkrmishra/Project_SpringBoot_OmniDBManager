...
/**
 * Postgres backup/restore via JDBC (no pg_dump binary required).
 * Format: gzip'd JSON {formatVersion:1, engine:"POSTGRES", database, backedUpAt, tables:[{name, columns, rows}]}
 * Rows are objects keyed by column name; restore truncates existing tables and inserts.
 */
@Service
...
public class PostgresBackupService {
    ...
    public BackupService.DatabaseBackupInfo describeDatabase(String dbName) {
        nameValidator.validatePostgresDatabaseName(dbName);
        boolean exists = postgresRepository.databaseExists(dbName);
        int tableCount = 0;
        if (exists) {
            try { tableCount = postgresRepository.listTables(dbName).size(); } catch (Exception ignored) {}
        }
    ...
    public BackupService.BackupResult writeBackup(String dbName, OutputStream out) {
        ...
        try (OutputStream gzip = new GZIPOutputStream(out)) {
            ...
            for (String table : tables) {
                ...
                firstTable = false;
                List<String> columns;
                try { columns = postgresRepository.getTableColumns(dbName, table); } catch (Exception e) {
                ...
                while (true) {
                    ...
                    try { rows = postgresRepository.listRows(dbName, table, batch, offset); } catch (Exception e) {
        ...
        log.info("Backed up POSTGRES database '{}': {} tables, {} rows", dbName, tables.size(), totalRows);
    ...
    public BackupService.RestoreResult restore(String dbName, byte[] content, boolean confirmed) {
        ...
        return locks.withLock(DatabaseEngineType.POSTGRES.name() + ":" + dbName, () -> {
            ...
            log.info("Restored POSTGRES database '{}': {} tables, {} rows", dbName, parsed.size(), totalRows);
    ...
    private List<ParsedTable> readBackup(byte[] content) {
        ...
        // Also accept legacy "collections" key for cross-compat
        if (tables.isEmpty()) {
            List<Document> cols = doc.getList("collections", Document.class);
            if (cols != null && !cols.isEmpty()) throw new NameNotAllowedException("Backup is for MongoDB, not PostgreSQL");
        }
