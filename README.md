...
- **Pooled connection profiles** — pooled connections select a named policy per connection: `standard` (default, existing pooler) or `high_concurrency` (dedicated `pgbouncer-hc:6433`, internal only, transaction pooling with larger capped sizing). Same endpoint, same credentials, same database — profile configuration lives per pooler instance, never in client input. Bare `mode=pooled` strings mean `standard`.
- **Single-host TLS bridge (pending cutover)** — optional Go proxy (`deploy/db-proxy/`) exposing one public port `:15432` for BOTH Postgres modes, routed by `options=-c omnidb.mode=<direct|pooled>` (+ optional `-c omnidb.pool_profile=<standard|high_concurrency>` for pooled) and stripped before the backend. Verify-full all backend legs, SNI-enforced, fail-closed, cert hot-reload; issued bridged strings pin `channel_binding=disable`. Live checklist: `deploy/database-bridge-gate.md`. Disabled by default (`DATABASE_PROXY_ENABLED=false`) until the gate passes.
...
postgresql://myapp_user:***@db.example.com:15432/myapp?sslmode=require&application_name=omnidb&channel_binding=disable&options=-c%20omnidb.mode%3Dpooled%20-c%20omnidb.pool_profile%3Dstandard
postgresql://myapp_user:***@db.example.com:15432/myapp?sslmode=require&application_name=omnidb&channel_binding=disable&options=-c%20omnidb.mode%3Dpooled%20-c%20omnidb.pool_profile%3Dhigh_concurrency
```

Pooled profiles: `DIRECT` (migrations/admin), `POOLED — Standard` (default runtime, transaction pooling), `POOLED — High Concurrency` (more pool headroom, still transaction pooling — not session affinity, same credentials). `GET /api/postgres/databases/{db}/connections` lists both profiles. `5432`/`6432`/`6433` stay internal; only `:15432` is public.
